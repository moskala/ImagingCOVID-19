...
class LungSegmentation_MethodA_dicom_unittest(unittest.TestCase):
    def test_load_scan(self):
        ...
        self.assertEqual(len(slices),len(os.listdir(Path(testDir))),"Number of slices should equal the number of dcm files in test folder")
        self.assertIsNotNone(slices)
        self.assertIsInstance(slices,list)

    def test_get_pixels_hu(self):
        lst = []
        lst.append(dicom.read_file(testFile))
        arr = SegmentationA.get_pixels_hu(lst)
        self.assertIsInstance(arr,np.ndarray)
        self.assertFalse(arr.any() == -2000)
    
    def test_seperate_lungs(self):
        pxls = np.stack(dicom.read_file(testFile).pixel_array)
